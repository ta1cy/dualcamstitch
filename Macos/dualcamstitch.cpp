...
cv::VideoCapture openCamera(int index, int width, int height, int fps) {
    cv::VideoCapture cap(index, cv::CAP_AVFOUNDATION);
    if (!cap.isOpened()) {
        cap.open(index);
    }
    if (!cap.isOpened()) {
        throw std::runtime_error("Failed to open camera index " + std::to_string(index));
    }
...
struct ShiftTimingInfo {
    double detect_ms = 0;    // ORB detectAndCompute (both images)
    double match_ms = 0;     // BFMatcher knnMatch + filtering
    int kp0 = 0;             // keypoints detected in img0
    int kp1 = 0;             // keypoints detected in img1
    int good_matches = 0;    // matches after ratio test
};
...
// -------------------------------------------------------------------------
// Windowed ORB: run feature matching on cropped overlap regions only,
// then translate the result back to full-frame coordinates.
//   - Crops rightmost windowFrac of img0  (the part likely to overlap)
//   - Crops leftmost  windowFrac of img1
//   - Runs ORB + BF match on those smaller images
//   - Adds cropOffset0 to convert dx back to full-frame space
// This reduces ORB computation while the stitch still uses full frames.
// -------------------------------------------------------------------------
bool estimateHorizontalShiftWindowed(const cv::Mat& img0, const cv::Mat& img1,
                                      int& dx, int& agree, float windowFrac,
                                      int maxFeatures = 1500, float ratio = 0.75f, bool debug = false,
                                      ShiftTimingInfo* timing = nullptr) {
    int w0 = img0.cols;
    int w1 = img1.cols;
    float frac = std::clamp(windowFrac, 0.1f, 1.0f);

    int keepLeft  = static_cast<int>(std::round(frac * w0));
    int keepRight = static_cast<int>(std::round(frac * w1));
    keepLeft  = std::clamp(keepLeft,  1, w0);
    keepRight = std::clamp(keepRight, 1, w1);

    // cropOffset0 = number of columns trimmed from the left side of img0
    int cropOffset0 = w0 - keepLeft;

    cv::Mat crop0 = img0(cv::Rect(cropOffset0, 0, keepLeft, img0.rows));
    cv::Mat crop1 = img1(cv::Rect(0, 0, keepRight, img1.rows));

    cv::Mat g0, g1;
    cv::cvtColor(crop0, g0, cv::COLOR_BGR2GRAY);
    cv::cvtColor(crop1, g1, cv::COLOR_BGR2GRAY);
    ...
    // Compute per-match dx in *cropped* coords
    std::vector<float> dxs;
    for (const auto& m : good) {
        float x0_crop = k0[m.queryIdx].pt.x;
        float x1_crop = k1[m.trainIdx].pt.x;
        dxs.push_back(x0_crop - x1_crop);
    }

    std::sort(dxs.begin(), dxs.end());
    float median = dxs[dxs.size() / 2];

    // Translate cropped dx → full-frame dx:
    //   full_x0 = crop_x0 + cropOffset0,  full_x1 = crop_x1  (no offset for img1)
    //   dx_full = full_x0 - full_x1 = dx_crop + cropOffset0
    dx = static_cast<int>(median) + cropOffset0;
    ...
    if (debug) {
        std::cout << "[shift-win] crop=" << keepLeft << "x" << keepRight
                  << " good=" << good.size() << " dx_crop=" << static_cast<int>(median)
                  << " dx_full=" << dx << " agree=" << agree << std::endl;
    }
...
int main(int argc, char** argv) {
    ...
    bool windowed = false;
    float windowFrac = 0.5f;   // fraction of each frame to keep for ORB (0.1 - 1.0)

    for (int i = 1; i < argc; i++) {
        std::string arg = argv[i];
        if (arg == "--cam0" && i + 1 < argc) cam0Idx = std::stoi(argv[++i]);
        else if (arg == "--cam1" && i + 1 < argc) cam1Idx = std::stoi(argv[++i]);
        ...
        else if (arg == "--window_frac" && i + 1 < argc) { windowFrac = std::stof(argv[++i]); windowed = true; }
    }
    ...
    cv::VideoCapture cap0 = openCamera(cam0Idx, width, height, fps);
    cv::VideoCapture cap1 = openCamera(cam1Idx, width, height, fps);
    ...
    std::cout << " w = toggle windowed ORB mode" << std::endl;
    std::cout << " [ / ] = shrink / grow ORB window (by 0.05)" << std::endl;
    ...
    if (g0.read(img0) && g1.read(img1)) {
        ...
        bool ok;
        if (windowed) {
            ok = estimateHorizontalShiftWindowed(img0c, img1c, dxTry, agreeTry, windowFrac, maxFeatures, ratio, debugShift);
        } else {
            ok = estimateHorizontalShift(img0c, img1c, dxTry, agreeTry, maxFeatures, ratio, debugShift);
        }
        if (ok && agreeTry >= minAgree) {
            dx = dxTry;
            agree = agreeTry;
            hasDx = true;
    ...
    // Timing accumulators for periodic stats
    ...
    while (true) {
        ...
        // --- periodic shift re-estimation (windowed or full) ---
        if (recalcEvery > 0 && (frameCount % recalcEvery == 0)) {
            int dxTry, agreeTry;
            bool ok;
            if (windowed) {
                ok = estimateHorizontalShiftWindowed(img0c, img1c, dxTry, agreeTry, windowFrac, maxFeatures, ratio, debugShift, &lastTiming);
            } else {
                ok = estimateHorizontalShift(img0c, img1c, dxTry, agreeTry, maxFeatures, ratio, debugShift, &lastTiming);
            }
            if (ok && agreeTry >= minAgree) {
        ...
        // All stitching uses the full-frame images
        ...
        char textBuf[256];
        snprintf(textBuf, sizeof(textBuf), "cam0 | cam1   FPS:%.1f%s", fpsVal, windowed ? "  [WIN]" : "");
        cv::putText(cams, textBuf, cv::Point(20, 40), cv::FONT_HERSHEY_SIMPLEX, 1.0,
                    cv::Scalar(255, 0, 0), 2, cv::LINE_AA);

        if (windowed) {
            snprintf(textBuf, sizeof(textBuf),
                     "dx:%d  agree:%d  seam:%.2f  blend:%d %s win:%d%%",
                     dx, agree, seamFrac, blendWidth,
                     autoSeam ? "(auto)" : "",
                     static_cast<int>(windowFrac * 100));
        } else {
            snprintf(textBuf, sizeof(textBuf),
                     "dx:%d  agree:%d  seam:%.2f  blend:%d %s",
                     dx, agree, seamFrac, blendWidth,
                     autoSeam ? "(auto)" : "");
        }
        ...
        if (key == 'q') {
            ...
            bool ok;
            if (windowed) {
                ok = estimateHorizontalShiftWindowed(img0c, img1c, dxTry, agreeTry, windowFrac, maxFeatures, ratio, true);
            } else {
                ok = estimateHorizontalShift(img0c, img1c, dxTry, agreeTry, maxFeatures, ratio, true);
            }
            if (ok && agreeTry >= minAgree) {
            ...
            windowed = !windowed;
            std::cout << "windowed ORB: " << (windowed ? "ON" : "OFF")
                      << " (frac=" << windowFrac << ")" << std::endl;
        } else if (key == '[' || key == ']') {
            float step = 0.05f;
            windowFrac += (key == ']') ? step : -step;
            windowFrac = std::clamp(windowFrac, 0.1f, 1.0f);
            std::cout << "window_frac: " << windowFrac << std::endl;
        }
    }
